...
var aoi = geom_bel; // should be a GEE geometry object - here we are getting it from an drawn polygon


// define years and dates to include in landsat image collection
var startYear  = 2012;    // what year do you want to start the time series 
...
var deg1  = 1;      // delta filter for 1 year duration disturbance, <= will not be included as disturbance - units are in units of segIndex defined in the following function definition
var deg20 = 2;      // delta filter for 20 year duration disturbance, <= will not be included as disturbance - units are in units of segIndex defined in the following function definition
var preVal     = 5;      // pre-disturbance value threshold - values below the provided threshold will exclude disturbance for those pixels - units are in units of segIndex defined in the following function definition
var mmu        = 5;       // minimum mapping unit for disturbance patches - units of pixels
...
var run_params = { 
  maxSegments:            6,     // have played w 3-6 segments 
  spikeThreshold:         0.5,
...
Map.setCenter(-88.5551, 17.1948, 7)                                           // center the map display and set zoom level
distImg = distImg.clip(aoi);                                                // clip the data to the geometry
...
Export.image.toDrive({
   ...
   region:geom_bel,
   folder:"GEE",
   description:"LS_distr_mag_2000_2017",
   scale:30,
   crs: "EPSG:4326",
   skipEmptyTiles: true,
...
Export.image.toDrive({
   ...
   region:geom_bel,
   folder:"GEE",
   description:"LS_distr_dur_2000_2017",
   scale:30,
   crs: "EPSG:4326",
   skipEmptyTiles: true,
   maxPixels:9000000000 //( pass max value if needed)
